    ...
    for ticker in ticker_list:
        ...
        try:
            ...
            if mcap >= min_mcap_absolute:
                ...
                # Unlevered Beta Calculation (assuming standard 25% corporate tax rate)
                unlevered_beta = None
                if beta is not None:
                    unlevered_beta = beta / (1 + (1 - 0.25) * de_ratio)
...
with tab2:
    ...
    if 'live_df' in st.session_state and not st.session_state['live_df'].empty:
        ...
        df["Unlevered_Beta"] = pd.to_numeric(df["Unlevered_Beta"], errors='coerce')
        ...
        with st.expander("⚙️ Screen & Filter Live Data", expanded=True):
            ...
            with col1:
                min_pe, max_pe = st.slider(
                    ...
                    (float(df["TTM_PE"].min()), float(df["TTM_PE"].max())), key="live_pe"
                )
            with col2:
                min_roe, max_roe = st.slider(
                    "ROE (%)", 
                    float(df["ROE_Pct"].min()), float(df["ROE_Pct"].max()), 
                    (float(df["ROE_Pct"].min()), float(df["ROE_Pct"].max())), key="live_roe"
                )
            with col3:
                ...
                if not growth_data.empty:
                    min_growth, max_growth = st.slider(
                        ...
                        (float(growth_data.min()), float(growth_data.max())), key="live_growth"
                    )
                else:
        ...
        if not filtered_df.empty:
            # High-Nuance Multi-Stage Gradient Heatmap
            fig2 = px.scatter(
                filtered_df, 
                ...
                color_continuous_scale=[
                    [0.0, "#1e3a8a"],   # Deep Navy
                    [0.2, "#0ea5e9"],   # Bright Sky Blue
                    [0.4, "#93c5fd"],   # Soft Light Blue
                    [0.5, "#e2e8f0"],   # Light Slate Grey
                    [0.6, "#fca5a5"],   # Soft Rose Red
                    [0.8, "#ef4444"],   # Vibrant Crimson
            ...
            if show_risk_plot:
                ...
                if not risk_df.empty:
                    melted_risk = risk_df.melt(
                        id_vars=["Ticker"], 
                        value_vars=["Beta", "Unlevered_Beta"], 
                    ...
                    fig_risk.update_traces(marker=dict(line=dict(width=1, color="#1e3a8a")))
                    st.plotly_chart(fig_risk, use_container_width=True, theme=None)
                else:
                    st.info("Beta data is not available for the currently screened stocks.")
            
            # Unlevered Valuation Toggle (EV/EBITDA)
            ...
            if show_valuation_plot:
                ...
                if not val_df.empty:
                    melted_val = val_df.melt(
                        id_vars=["Ticker"], 
                        value_vars=["TTM_PE", "EV_EBITDA"], 
                        var_name="Valuation_Metric", 
                        value_name="Multiple"
                    )
                    fig_val = px.bar(
                    ...
                    fig_val.update_traces(marker=dict(line=dict(width=1, color="#1e3a8a")))
                    st.plotly_chart(fig_val, use_container_width=True, theme=None)
                else:
                    st.info("EV/EBITDA data is not available for the currently screened stocks.")
